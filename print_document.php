<?php
// Document Print & PDF Generator for Client Portal (PHP 5.6 Compatible)
...
// Determine default administrator / attending technician name
$admin_name = '';
try {
    $stmt_tn = $pdo->prepare("SELECT techname FROM bucket_technotes WHERE accountnum = :acct AND TRIM(techname) != '' ORDER BY id DESC LIMIT 1");
    $stmt_tn->execute(array(':acct' => $accountnum));
    $tn_tech = $stmt_tn->fetchColumn();
    if ($tn_tech) {
        $admin_name = resolve_tech_fullname($pdo, $tn_tech, '');
    }
} catch (PDOException $e) {}
...
$doc_ref = '';
$doc_date = date('Y-m-d');
$client_acct = $accountnum;
$doc_status = 'Completed';
$tech_name = $admin_name;
$data = null;

// 1. Fetch document based on type (STRICTLY scoped to logged client account)
if ($doc_type === 'workorder') {
    $stmt = $pdo->prepare("SELECT * FROM bucket_workorder WHERE id = :id AND accountnum = :acct LIMIT 1");
    $stmt->execute(array(':id' => $doc_id, ':acct' => $accountnum));
    $data = $stmt->fetch();

    if (!$data) {
        die("Work Order statement not found or access denied.");
    }
    ...
    $stmt = $pdo->prepare("SELECT * FROM bucket_technotes WHERE id = :id AND accountnum = :acct LIMIT 1");
    $stmt->execute(array(':id' => $doc_id, ':acct' => $accountnum));
    $data = $stmt->fetch();

    if (!$data) {
        die("Technical Service Note not found or access denied.");
    }
    ...
    $stmt = $pdo->prepare("SELECT l.*, i.name as item_name, i.item_code, i.category, i.selling_price 
        ...
        WHERE l.id = :id AND l.accountnum = :acct LIMIT 1");
    $stmt->execute(array(':id' => $doc_id, ':acct' => $accountnum));
    $data = $stmt->fetch();

    if (!$data) {
        die("Pull-Out / Movement Record not found or access denied.");
    }
    ...
    $stmt = $pdo->prepare("SELECT * FROM hardware_troubleshooting_logs WHERE id = :id AND accountnum = :acct LIMIT 1");
    $stmt->execute(array(':id' => $doc_id, ':acct' => $accountnum));
    $data = $stmt->fetch();

    if (!$data) {
        die("Diagnostic Log not found or access denied.");
    }
