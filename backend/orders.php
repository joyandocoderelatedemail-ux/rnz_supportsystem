...
<head>
    ...
    <title><?php echo sanitize($page_title); ?> - RNZ Admin</title>
    <!-- Tailwind CSS CDN -->
...
<div class="flex min-h-screen">
    ...
    <div class="flex-1 flex flex-col min-w-0">
        ...
        <main class="p-6 sm:p-8 pb-24 md:pb-8 space-y-6 sm:space-y-8 max-w-7xl w-full mx-auto">

            <!-- Flash Notifications -->
            <?php if (!empty($flash_msg)): ?>
                <div class="p-4 rounded-2xl bg-emerald-50 border border-emerald-200 text-emerald-800 text-xs sm:text-sm font-bold flex items-center justify-between shadow-sm animate-in fade-in duration-200">
            ...
            <?php if (!empty($flash_err)): ?>
                <div class="p-4 rounded-2xl bg-rose-50 border border-rose-200 text-rose-800 text-xs sm:text-sm font-bold flex items-center justify-between shadow-sm animate-in fade-in duration-200">
                    <div class="flex items-center space-x-2">
            ...
            <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div class="space-y-1">
                    <div class="flex items-center space-x-2.5">
                        <span class="px-2.5 py-0.5 rounded-full text-[10px] font-bold bg-[#EB3E0B]/10 text-[#EB3E0B] border border-[#EB3E0B]/20 uppercase tracking-wide">
                            Fulfillment & Logistics
                        </span>
                        <?php if ($pending_orders_count > 0): ?>
                            <span class="px-2.5 py-0.5 rounded-full text-[10px] font-bold bg-amber-100 text-amber-800 border border-amber-200 animate-pulse">
                                <?php echo $pending_orders_count; ?> Pending Approval
                            </span>
                        <?php endif; ?>
                    </div>
                    <h1 class="text-2xl sm:text-3xl font-extrabold text-slate-900 tracking-tight">Client Hardware Orders</h1>
                    <p class="text-xs sm:text-sm text-slate-500">Review client supply orders, dispatch materials, and track fulfillment status.</p>
                </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-5">
                ...
                <div class="bg-white rounded-3xl p-5 sm:p-6 border border-slate-200 shadow-sm flex items-center justify-between">
                    <div class="space-y-1">
                        ...
                        <h3 class="text-3xl font-extrabold text-slate-900 font-mono"><?php echo number_format($total_orders_count); ?></h3>
                        <p class="text-[11px] text-slate-400">All registered material requests</p>
                    </div>
                    <div class="w-12 h-12 rounded-2xl bg-blue-50 text-blue-600 border border-blue-100 flex items-center justify-center font-bold">
                ...
                <!-- Pending Orders -->
                <a href="orders.php?status=Pending" class="bg-white hover:bg-amber-50/50 rounded-3xl p-5 sm:p-6 border border-slate-200 hover:border-amber-400 shadow-sm flex items-center justify-between transition-all group">
                    <div class="space-y-1">
                        <span class="text-xs font-bold text-amber-600 uppercase tracking-wider">Pending Orders</span>
                        <h3 class="text-3xl font-extrabold text-amber-600 font-mono"><?php echo number_format($pending_orders_count); ?></h3>
                        <p class="text-[11px] text-slate-400 group-hover:text-amber-700">Requires technical review</p>
                    </div>
                    <div class="w-12 h-12 rounded-2xl bg-amber-50 text-amber-600 border border-amber-100 flex items-center justify-center font-bold">
                ...
                <a href="orders.php?status=Processing" class="bg-white hover:bg-indigo-50/50 rounded-3xl p-5 sm:p-6 border border-slate-200 hover:border-indigo-400 shadow-sm flex items-center justify-between transition-all group">
                    <div class="space-y-1">
                        <span class="text-xs font-bold text-indigo-600 uppercase tracking-wider">In Transit / Dispatch</span>
                        <h3 class="text-3xl font-extrabold text-indigo-600 font-mono"><?php echo number_format($in_transit_count); ?></h3>
                        <p class="text-[11px] text-slate-400 group-hover:text-indigo-700">Approved or out for delivery</p>
                    </div>
                    <div class="w-12 h-12 rounded-2xl bg-indigo-50 text-indigo-600 border border-indigo-100 flex items-center justify-center font-bold">
                ...
                <div class="bg-white rounded-3xl p-5 sm:p-6 border border-slate-200 shadow-sm flex items-center justify-between">
                    <div class="space-y-1">
                        <span class="text-xs font-bold text-emerald-600 uppercase tracking-wider">Total Orders Value</span>
                        <h3 class="text-2xl sm:text-3xl font-extrabold text-emerald-600 font-mono truncate">₱<?php echo number_format($total_order_revenue, 2); ?></h3>
                        <p class="text-[11px] text-slate-400">Gross materials requested</p>
                    </div>
                    <div class="w-12 h-12 rounded-2xl bg-emerald-50 text-emerald-600 border border-emerald-100 flex items-center justify-center font-bold">
            ...
            <div class="bg-white rounded-3xl border border-slate-200 shadow-sm p-5 sm:p-6 space-y-4">
                <form method="GET" action="" class="grid grid-cols-1 sm:grid-cols-12 gap-3">
                    ...
                    <div class="sm:col-span-5 relative">
                        <input type="text" name="search" value="<?php echo sanitize($search); ?>" placeholder="Search by Order #, Client Name, Acct #, or Material SKU..." class="w-full bg-slate-50 text-slate-800 text-xs pl-10 pr-4 py-3 rounded-2xl border border-slate-200 focus:border-[#EB3E0B] focus:bg-white focus:outline-none transition-all placeholder-slate-400">
                    ...
                    <div class="sm:col-span-3">
                        <select name="status" class="w-full bg-slate-50 text-slate-800 text-xs px-4 py-3 rounded-2xl border border-slate-200 focus:border-[#EB3E0B] focus:bg-white focus:outline-none transition-all font-semibold">
                    ...
                    <!-- Sort Filter -->
                    <div class="sm:col-span-3">
                        <select name="sort" class="w-full bg-slate-50 text-slate-800 text-xs px-4 py-3 rounded-2xl border border-slate-200 focus:border-[#EB3E0B] focus:bg-white focus:outline-none transition-all font-semibold">
                    ...
                    <div class="sm:col-span-1 flex items-center gap-1.5">
                        ...
                        <?php if (!empty($search) || !empty($status_filter) || $sort_by !== 'newest'): ?>
                            <a href="orders.php" class="p-3 bg-slate-100 hover:bg-slate-200 text-slate-500 hover:text-slate-700 rounded-2xl text-xs font-bold transition-all flex items-center justify-center shrink-0" title="Reset Filters">
                                &times;
                            </a>
                        <?php endif; ?>
                    </div>
            ...
            <div class="bg-white rounded-3xl border border-slate-200 shadow-sm overflow-hidden space-y-4">
                <div class="p-6 border-b border-slate-100 flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                    <div>
                        <h3 class="text-lg font-extrabold text-slate-900">Registered Orders List</h3>
                        <p class="text-xs text-slate-500 mt-0.5">Manage customer deliveries, fulfillment approval, and inventory sync.</p>
                    </div>
                    <div class="text-xs text-slate-500 font-medium">
                        Showing <strong class="text-slate-900 font-mono"><?php echo count($orders_list); ?></strong> order(s)
                    </div>
                </div>
                            ...
                            <?php if (empty($orders_list)): ?>
                                <tr>
                                    <td colspan="8" class="py-12 text-center text-slate-400 space-y-3">
                                        ...
                                        <p class="font-bold text-slate-600">No hardware orders found.</p>
                                        <p class="text-xs text-slate-400">Try clearing search filters or click "Create Client Order" to create a new order.</p>
                                    </td>
                                </tr>
                            <?php else: ?>
                                <?php foreach ($orders_list as $ord): 
                                    $status = $ord['status'];
                                    if ($status === 'Pending') {
                                        $badge_cls = 'bg-amber-100 text-amber-800 border-amber-200';
                                    } elseif ($status === 'Approved' || $status === 'Processing') {
                                        $badge_cls = 'bg-blue-100 text-blue-800 border-blue-200';
                                    } elseif ($status === 'Out for Delivery') {
                                        $badge_cls = 'bg-purple-100 text-purple-800 border-purple-200';
                                ...
                                ?>
                                    <tr class="hover:bg-slate-50/80 transition-colors">
                                        <!-- Order # & Date -->
                                        <td class="py-4 px-6">
                                            <span class="font-mono font-extrabold text-[#EB3E0B] text-xs block">
                                        ...
                                        <td class="py-4 px-6">
                                            ...
                                            <span class="font-mono text-[10px] text-slate-500 font-semibold block">
                                                SKU: <?php echo sanitize($ord['item_code']); ?>
                                            </span>
                                            <?php if (!empty($ord['notes'])): ?>
                                                <p class="text-[11px] text-slate-600 italic mt-1 bg-slate-50 p-2 rounded-xl border border-slate-200 max-w-xs">
                                        ...
                                        <td class="py-4 px-6 text-center">
                                            <span class="font-mono font-extrabold text-sm text-slate-800 bg-slate-100 px-2.5 py-1 rounded-xl">
                                        ...
                                        <td class="py-4 px-6 text-xs text-slate-600 max-w-xs">
                                            <p class="font-bold text-slate-800"><?php echo sanitize($ord['contact_person']); ?> (<?php echo sanitize($ord['contact_number']); ?>)</p>
                                            <p class="truncate text-[11px] text-slate-500 mt-0.5" title="<?php echo sanitize($ord['delivery_address']); ?>">
                                        ...
                                        <td class="py-4 px-6 text-right">
                                            <?php if ($my_tier >= 2): ?>
                                                <div class="flex items-center justify-end space-x-1.5">
                                                    <button type="button" onclick='openStatusModal(<?php echo htmlspecialchars(json_encode($ord), ENT_QUOTES, 'UTF-8'); ?>)' class="px-3 py-1.5 rounded-xl bg-slate-100 hover:bg-[#EB3E0B] text-slate-700 hover:text-white font-bold text-[11px] transition-colors shadow-xs" title="Update Status & Fulfillment">
                                                        Update
                                                    </button>
                                                    ...
                                                    <form method="POST" action="" onsubmit="return confirm('Are you sure you want to delete order <?php echo addslashes($ord['order_number']); ?>?');" class="inline">
                                                        ...
                                                        <button type="submit" class="w-8 h-8 rounded-xl bg-slate-100 hover:bg-rose-50 text-slate-400 hover:text-rose-600 flex items-center justify-center transition-colors" title="Delete Order">
...
<div id="statusModal" class="fixed inset-0 z-50 bg-slate-900/60 backdrop-blur-sm hidden items-center justify-center p-4 overflow-y-auto">
    <div class="bg-white rounded-3xl shadow-2xl border border-slate-200 max-w-xl w-full p-6 sm:p-8 space-y-6 my-8 animate-in fade-in zoom-in duration-150 text-slate-800">
        <div class="flex items-center justify-between border-b border-slate-100 pb-4">
            <div class="flex items-center space-x-3">
                ...
                <div>
                    <h3 class="font-extrabold text-lg text-slate-900">Update Order Fulfillment</h3>
                    <p class="text-xs text-slate-500" id="modal_order_num_display">Order Number</p>
                </div>
            </div>
        ...
        <form method="POST" action="" class="space-y-4">
            ...
            <div class="p-4 bg-slate-50 rounded-2xl border border-slate-200 space-y-2 text-xs">
                <div class="flex justify-between items-center">
                    ...
                    <strong id="modal_client_display" class="text-slate-900 font-sans"></strong>
                </div>
                <div class="flex justify-between items-center">
                    <span class="text-slate-500">Item & Qty:</span>
                    <strong id="modal_item_display" class="text-[#EB3E0B] font-mono"></strong>
                </div>
                <div class="flex justify-between items-center">
                    <span class="text-slate-500">Total Amount:</span>
                    <strong id="modal_amount_display" class="text-emerald-700 font-mono text-sm"></strong>
                </div>
            </div>

            <!-- Status Selector -->
            <div class="space-y-1">
                <label class="text-xs font-bold text-slate-700">Order Status <span class="text-[#EB3E0B]">*</span></label>
                <select name="status" id="modal_status_select" required class="w-full bg-slate-50 text-slate-900 text-xs px-4 py-3 rounded-2xl border border-slate-200 focus:border-[#EB3E0B] focus:bg-white focus:outline-none font-bold">
            ...
            <div class="space-y-1">
                <label class="text-xs font-bold text-slate-700">Dispatch / Technician Notes</label>
                <textarea name="admin_remarks" id="modal_remarks_input" rows="2" placeholder="e.g. Dispatched via Tech John on service van; or Waybill # LBC-984210" class="w-full bg-slate-50 text-slate-900 text-xs p-3 rounded-2xl border border-slate-200 focus:border-[#EB3E0B] focus:bg-white focus:outline-none"></textarea>
            </div>
            ...
            <div class="p-3.5 bg-slate-50 border border-slate-200 rounded-2xl space-y-1">
                ...
                <p class="text-[10px] text-slate-500 ml-6">Automatically creates a movement log in inventory hub tagged to this client account.</p>
            </div>

            <!-- Level 2 Access Code -->
            <?php if ($my_tier === 2): ?>
                <div class="p-3.5 bg-amber-50 border border-amber-200 rounded-2xl space-y-1.5">
                    <label class="text-xs font-bold text-amber-800 flex items-center space-x-1.5">
                    ...
                    <input type="password" name="action_access_code" required placeholder="Enter 4-digit security code" class="w-full bg-white text-slate-900 text-xs px-3.5 py-2.5 rounded-xl border border-amber-200 focus:border-amber-500 focus:outline-none font-mono">
                </div>
            <?php endif; ?>

            <div class="pt-4 border-t border-slate-100 flex items-center justify-end space-x-3">
                <button type="button" onclick="closeStatusModal()" class="px-5 py-2.5 rounded-2xl bg-slate-100 text-slate-700 font-bold text-xs hover:bg-slate-200 transition-colors">
                    Cancel
                </button>
...
<!-- ========================================================================= -->
<div id="newOrderModal" class="fixed inset-0 z-50 bg-slate-900/60 backdrop-blur-sm hidden items-center justify-center p-4 overflow-y-auto">
    <div class="bg-white rounded-3xl shadow-2xl border border-slate-200 max-w-xl w-full p-6 sm:p-8 space-y-6 my-8 animate-in fade-in zoom-in duration-150 text-slate-800">
        <div class="flex items-center justify-between border-b border-slate-100 pb-4">
            <div class="flex items-center space-x-3">
                <div class="w-10 h-10 rounded-2xl bg-[#EB3E0B] text-white flex items-center justify-center font-bold shadow-md shadow-[#EB3E0B]/30">
                ...
                <div>
                    <h3 class="font-extrabold text-lg text-slate-900">Create Client Hardware Order</h3>
                    <p class="text-xs text-slate-500">Order materials on behalf of a store client.</p>
                </div>
            </div>
            <button type="button" onclick="closeNewOrderModal()" class="text-slate-400 hover:text-slate-600 p-2 rounded-full hover:bg-slate-100">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/></svg>
            </button>
        </div>
        ...
        <form method="POST" action="" class="space-y-4">
            ...
            <!-- 1. Client Selection -->
            <div class="space-y-1">
                <label class="text-xs font-bold text-slate-700">Client Account <span class="text-[#EB3E0B]">*</span></label>
                <select name="accountnum" required onchange="handleAdminClientSelect(this)" class="w-full bg-slate-50 text-slate-900 text-xs px-4 py-3 rounded-2xl border border-slate-200 focus:border-[#EB3E0B] focus:bg-white focus:outline-none font-bold">
            ...
            <!-- 2. Hardware Item -->
            <div class="space-y-1">
                <label class="text-xs font-bold text-slate-700">Hardware Material <span class="text-[#EB3E0B]">*</span></label>
                <select name="item_id" id="admin_order_item_id" required onchange="calculateAdminOrderTotal()" class="w-full bg-slate-50 text-slate-900 text-xs px-4 py-3 rounded-2xl border border-slate-200 focus:border-[#EB3E0B] focus:bg-white focus:outline-none font-bold">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 items-center">
                <div class="space-y-1">
                    ...
                    <input type="number" name="quantity" id="admin_order_quantity" min="1" value="1" required oninput="calculateAdminOrderTotal()" class="w-full bg-slate-50 text-slate-900 text-sm font-extrabold px-4 py-2.5 rounded-2xl border border-slate-200 focus:border-[#EB3E0B] focus:bg-white focus:outline-none font-mono">
                </div>
                <div class="p-3 bg-slate-50 border border-slate-200 rounded-2xl flex flex-col justify-center">
                    <span class="text-[10px] font-bold text-slate-500 uppercase tracking-wider">Estimated Total</span>
                    <span id="admin_total_display" class="text-xl font-extrabold font-mono text-emerald-700">₱0.00</span>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div class="space-y-1">
                    <label class="text-xs font-bold text-slate-700">Contact Person</label>
                    <input type="text" name="contact_person" id="admin_contact_person" class="w-full bg-slate-50 text-slate-900 text-xs px-4 py-2.5 rounded-2xl border border-slate-200 focus:border-[#EB3E0B] focus:bg-white focus:outline-none">
                </div>
                <div class="space-y-1">
                    <label class="text-xs font-bold text-slate-700">Contact Phone #</label>
                    <input type="text" name="contact_number" id="admin_contact_number" class="w-full bg-slate-50 text-slate-900 text-xs px-4 py-2.5 rounded-2xl border border-slate-200 focus:border-[#EB3E0B] focus:bg-white focus:outline-none font-mono">
                </div>
            </div>

            <!-- 5. Delivery Address -->
            <div class="space-y-1">
                <label class="text-xs font-bold text-slate-700">Delivery Address</label>
                <textarea name="delivery_address" id="admin_delivery_address" rows="2" class="w-full bg-slate-50 text-slate-900 text-xs p-3 rounded-2xl border border-slate-200 focus:border-[#EB3E0B] focus:bg-white focus:outline-none"></textarea>
            </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div class="space-y-1">
                    ...
                    <select name="status" class="w-full bg-slate-50 text-slate-900 text-xs px-4 py-2.5 rounded-2xl border border-slate-200 focus:border-[#EB3E0B] focus:bg-white focus:outline-none font-medium">
                ...
                <div class="space-y-1">
                    <label class="text-xs font-bold text-slate-700">Payment Terms</label>
                    <select name="payment_method" class="w-full bg-slate-50 text-slate-900 text-xs px-4 py-2.5 rounded-2xl border border-slate-200 focus:border-[#EB3E0B] focus:bg-white focus:outline-none font-medium">
            ...
            <!-- 7. Notes -->
            <div class="space-y-1">
                <label class="text-xs font-bold text-slate-700">Dispatch / Admin Notes</label>
                <input type="text" name="admin_remarks" placeholder="e.g. Prepared for tomorrow morning route" class="w-full bg-slate-50 text-slate-900 text-xs px-4 py-2.5 rounded-2xl border border-slate-200 focus:border-[#EB3E0B] focus:bg-white focus:outline-none">
            </div>
            ...
            <div class="pt-4 border-t border-slate-100 flex items-center justify-end space-x-3">
                <button type="button" onclick="closeNewOrderModal()" class="px-5 py-2.5 rounded-2xl bg-slate-100 text-slate-700 font-bold text-xs hover:bg-slate-200 transition-colors">
                    Cancel
                </button>
                <button type="submit" class="px-6 py-2.5 rounded-2xl bg-[#EB3E0B] hover:bg-[#C32C0B] text-white font-extrabold text-xs shadow-md shadow-[#EB3E0B]/30 transition-all active:scale-95">
                    Create Order
