...
$acct_param = isset($_GET['acct']) ? trim($_GET['acct']) : (isset($_GET['accountnum']) ? trim($_GET['accountnum']) : '');
$autoprint = isset($_GET['autoprint']) && $_GET['autoprint'] == '1';

if (empty($doc_type) || ($doc_type !== 'soa' && $doc_id <= 0) || ($doc_type === 'soa' && empty($acct_param) && $doc_id <= 0)) {
    die("Invalid document type, ID, or account number.");
}
...
if ($doc_type === 'workorder') {
    ...
    $client_acct = !empty($acct_param) ? $acct_param : '';
    if (empty($client_acct) && $doc_id > 0) {
        $stmt_find = $pdo->prepare("SELECT accountnum FROM bucket_client WHERE id = :id LIMIT 1");
        $stmt_find->execute(array(':id' => $doc_id));
        $client_acct = $stmt_find->fetchColumn();
    }
    if (empty($client_acct)) {
        die("Client account number required for Statement of Account.");
    }
    ...
    $client = $stmt_c->fetch();
    if (!$client) {
        die("Client account #" . htmlspecialchars($client_acct) . " not found.");
    }
    ...
    $stmt_ho->execute(array(':acct' => $client_acct));
    ...
    $doc_ref = 'SOA-' . $client_acct . '-' . date('Ymd');
    $doc_date = date('Y-m-d');
...
<body class="bg-slate-100 text-slate-800 antialiased p-3 sm:p-6 min-h-screen flex flex-col items-center justify-start">
    ...
    <div id="printDocumentCard" class="print-page bg-white rounded-2xl p-4 sm:py-[0.3in] sm:px-[0.5in] border border-slate-200 shadow-xl max-w-4xl w-full text-slate-800 space-y-5">
        ...
        <!-- 6. STATEMENT OF ACCOUNT (SOA) BODY -->
        <!-- 6. STATEMENT OF ACCOUNT (SOA) BODY -->
        <?php if ($doc_type === 'soa'): ?>
            <div class="space-y-4">
                ...
                <?php if (!$has_any_pending): ?>
                    <div class="p-8 text-center border border-emerald-200 bg-emerald-50 rounded-2xl text-emerald-800 space-y-1">
                        ...
                        <p class="text-xs text-emerald-700">All itemized special services, equipment advances, and maintenance work orders for this account are fully settled.</p>
                    </div>
                <?php endif; ?>
