                                        ...
                                        <div class="mt-2.5 pt-2 border-t <?php echo $is_tech ? 'border-[#FFE8D5]' : 'border-slate-200'; ?>">
                                            <div class="flex flex-wrap gap-2.5">
                                                <?php foreach ($att_list as $img_rel): ?>
                                                    <?php echo build_chat_attachment_html($img_rel, '../', $is_tech ? 'border-[#FECDAA]' : 'border-slate-200'); ?>
                                                <?php endforeach; ?>
                                            </div>
                                        </div>
                                    <?php endif; ?>
...
function buildChatAttachmentHtmlJs(url, imgBorder) {
    ...
    return '<a href="' + safeUrl + '" target="_blank" rel="noopener" ' +
        ...
        '</span>' +
        '<span class="min-w-0">' +
            '<span class="block text-[11px] font-bold text-slate-800 truncate">' + escapeHtml(typeLabel) + '</span>' +
...
function buildTechReplyCard(reply) {
    ...
    if (atts.length > 0) {
        ...
        for (var i = 0; i < atts.length; i++) {
            ...
            var imgUrl = '../' + rawPath;
            contentHtml += buildChatAttachmentHtmlJs(imgUrl, borderClr);
        }
        contentHtml += '</div></div>';
    }
